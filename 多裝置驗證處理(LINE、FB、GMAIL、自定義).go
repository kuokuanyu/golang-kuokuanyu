...
var (
	...
	oauthStateString = "actionis%sandactivity_idis%sanddeviceis%sandgame_idis%s" // state驗證參數，用於防止CSRF攻擊
	...
	if device == "line" {
		...
		ctx.Redirect(http.StatusFound,
			NewSocialClient(ctx, lineChannelID, lineChannelSecret).GetWebLoinURL(redirectURL,
				fmt.Sprintf(oauthStateString, action, activityID, device, gameID),
		...
		// facebbok裝置
		ctx.Redirect(http.StatusFound,
			facebookOauthConfig.AuthCodeURL(fmt.Sprintf(oauthStateString, action, activityID, device, gameID)))
		return
		...
		ctx.Redirect(http.StatusFound,
			googleOauthConfig.AuthCodeURL(fmt.Sprintf(oauthStateString, action, activityID, device, gameID), oauth2.AccessTypeOffline))
		return
...
func (h *Handler) LoginCallback(ctx *gin.Context) {
	...
	// actionis%sandactivity_idis%sanddeviceis%sandgame_idis%s
	if len(state) > 8 {
		states := strings.Split(state[8:], "andactivity_idis") // [action資料, activity_id資料anddeviceis%sandgame_idis%s]

		// 取得action資料
		action = states[0]

		if len(states) > 1 {
			states = strings.Split(states[1], "anddeviceis") // [activity_id資料, device資料andgame_idis%s]
		...
		if len(states) > 1 {
			states = strings.Split(states[1], "andgame_idis") // [device資料, game_id資料]
	...
	if device == "line" || device == "facebook" || device == "gmail" {
		// 防止被攻擊，state判斷
		if state != fmt.Sprintf(oauthStateString, action, activityID, device, gameID) {
	...
	if device == "line" {
		...
		if action == "bind" {
			...
			userModel, err := models.DefaultUserModel().
				SetConn(h.dbConn, h.redisConn, h.mongoConn).
				Find(true, true, "users.user_id", ctx.Query("user_id"))
			if err != nil {
				h.executeErrorHTML(ctx, "錯誤: 取得用戶資料發生問題")
				return
			}
	...
	if device == "line" {
		...
		if action == "bind" {
			// line綁定功能(有user_id參數)
			var message string
			redirectURL = fmt.Sprintf(config.HTTPS_ACTIVITY_URL, config.HILIVES_NET_URL, "true")

			if err = models.DefaultLineModel().
				SetConn(h.dbConn, h.redisConn, h.mongoConn).
				UpdateIdentify(userID, ctx.Query("user_id")); err != nil {
				message = "管理員與LINE用戶綁定過程中發生錯誤，可能原因: 該管理員已與其他LINE用戶綁定"
			} else {
				message = "管理員與LINE用戶綁定成功!"

				// 先判斷redis裡是否有用戶資訊，有則修改identify資料
				...
				if err := models.DefaultUserModel().
					SetConn(h.dbConn, h.redisConn, h.mongoConn).
					Update(true, models.EditUserModel{UserID: ctx.Query("user_id"), Bind: "yes"},
						"users.user_id", ctx.Query("user_id")); err != nil {
					message = "管理員與LINE用戶綁定過程中發生錯誤，可能原因: 該管理員已與其他LINE用戶綁定"
				}
			}
